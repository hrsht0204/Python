...
def is_thumbs_up(landmarks):
    """Check if it's a thumbs-up gesture with some tolerance for angle misalignment."""
    thumb_tip = landmarks[4].y
    thumb_ip = landmarks[3].y  # Interphalangeal joint of the thumb
    index_tip = landmarks[8].y
    index_dip = landmarks[7].y  # Distal interphalangeal joint of the index finger
    index_pip = landmarks[6].y  # Proximal interphalangeal joint of the index finger

    # Define tolerance range for angle differences
    thumb_angle_tolerance = 15  # Tolerance for angle misalignment

    # Thumb is raised above index, and index is curled downward
    if thumb_tip < index_tip and thumb_ip < index_dip and index_pip < index_dip:
        return True
    elif thumb_tip < index_tip + thumb_angle_tolerance and thumb_ip < index_dip + thumb_angle_tolerance and index_pip < index_dip + thumb_angle_tolerance:
        return True  # Allow slight misalignment in angles

    return False

def is_thumbs_down(landmarks):
    """Check if it's a thumbs-down gesture with some tolerance for angle misalignment."""
    thumb_tip = landmarks[4].y
    thumb_base = landmarks[2].y  # Base of the thumb
    index_tip = landmarks[8].y
    ...
    # Thumb is pointing downwards
    if thumb_tip > thumb_base and index_tip < thumb_tip and middle_tip < thumb_tip:
        return True
    elif thumb_tip > thumb_base - thumb_angle_tolerance and index_tip < thumb_tip + thumb_angle_tolerance and middle_tip < thumb_tip + thumb_angle_tolerance:
        return True  # Allow slight misalignment in angles
        ...
        for hand_landmarks, hand_info in zip(results.multi_hand_landmarks, results.multi_handedness):
            ...
            if is_thumbs_up(hand_landmarks.landmark):
                    if not desktop_shown:
                        ...
                        print("Thumbs-up detected: Showing desktop")
                        desktop_shown = True  # Mark that desktop is shown
                    else:
                        print("Desktop is already shown")
                ...
                if desktop_shown:
                    pyautogui.hotkey('win', 'd')  # Undo the show desktop (bring back previous window)
                    print("Thumbs-down detected: Undoing desktop show")
                    desktop_shown = False  # Mark that desktop is no longer shown
                else:
